...
const page = async ({ params }: { params: Props }) => {
  ...
  if (slug.length === 1) {
    ...
    const catRes = await apiGetProductCategoryUniqueId(
      slug[1],
    ...
    if (productId) {
      const response = (
        await apiGetProductDetails(companyData.unique_id, productId, slug[0])
      ).data;
      if (response == null || !response?.category) return notFound();
      return redirect(
        `/products/${slug[0]}/${response?.category?.slug}/${slug[1]}`,
      );
    } else {
