          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-400 text-[11px] md:text-xs m-0 mt-1 leading-relaxed">
              ...
              <span className="hidden md:inline ml-1 font-medium text-gray-500">
                Last verified: {getLastUpdateDate(1)}
              </span>
            </p>
          </div>
        </div>
        ...
        {offers && offers.length > 0 ? (
          offers.map((item: OffersOffer, i: number) => {
            return (
              <div
                key={i}
                className="flex flex-col h-full w-full transition-all duration-500 hover:-translate-y-2"
              >
                <OfferCard
                  offer={item}
          ...
          <div className="col-span-full w-full py-24 bg-[#fcfcfc] rounded-[3rem] border border-dashed border-gray-200 flex flex-col items-center justify-center text-center px-6">
            <div className="relative w-24 h-24 mb-6">
              ...
              <div className="relative w-full h-full bg-white rounded-full flex items-center justify-center border border-gray-100 shadow-sm">
                <span className="text-3xl grayscale opacity-30 select-none">
                  🔍
                </span>
              </div>
            </div>
            <h3 className="text-2xl font-black text-[#1a1612] m-0 tracking-tight">
              No Offers Found
            </h3>
            <p className="text-gray-400 mt-3 max-w-sm text-sm leading-relaxed">
              We couldn&apos;t find any active deals in this section. Try
              exploring our other categories for massive savings!
            </p>
          </div>
        )}
      </div>
