...
import { getBaseImageUrl } from '@/constants/hooks'; // Tumhari original hook import
    ...
    return (
        <div className="relative w-full h-[350px] md:h-[450px] flex items-center justify-center overflow-hidden perspective-1000">
            {categories.map((category, index) => {
                // Infinite loop ka formula (Center, Left, Right align karne ke liye)
                let diff = index - activeIndex;
                if (diff > total / 2) diff -= total;
                if (diff < -total / 2) diff += total;

                // Center, Adjacent 1, Adjacent 2 positions check
                ...
                // Core 3D Logic without any library
                if (isCenter) {
                    translateX = 0;
                    scale = 1;
                    zIndex = 30;
                    ...
                    translateX = diff > 0 ? 120 : -120; // X axis movement
                    scale = 0.8; // Thora chota
                    zIndex = 20;
                    opacity = 0.6;
                    pointerEvents = 'auto'; // Click karke isko center me lane ke lye
                } else if (isSecondAdjacent) {
                    translateX = diff > 0 ? 220 : -220; // Aur dur
                    scale = 0.6; // Aur chota
                    zIndex = 10;
                    opacity = 0.3;
                } else {
                    translateX = diff > 0 ? 300 : -300; // Hidden out of bounds
                    scale = 0.4;
                    zIndex = 0;
                    opacity = 0;
                }
                ...
                return (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className={`absolute inset-0 bg-gradient-to-b from-[#FF5722]/5 to-transparent transition-opacity duration-500 ${isCenter ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />

                            <div className="relative w-16 h-16 mb-2 flex items-center justify-center z-10">
                                {/* Light Glow Effect */}
                                <div className={`absolute inset-0 bg-[#FF5722]/10 blur-xl rounded-full transition-transform duration-500 ${isCenter ? 'scale-150' : 'scale-0 group-hover:scale-125'}`} />
                                ...
                                {category.category_image ? (
                                    ...
                                    <div className={`relative z-10 w-14 h-14 rounded-xl border flex items-center justify-center font-black text-2xl transition-all duration-500
                            ...
                            <span className={`relative z-10 text-[14px] font-bold text-center uppercase tracking-tight leading-tight transition-colors duration-300
                            ...
                            {/* Bottom Accent Line */}
                            <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-[4px] transition-all duration-500 rounded-t-full bg-[#FF5722] ${isCenter ? 'w-1/2' : 'w-0 group-hover:w-1/4'}`} />
                        </div>
                    </div>
                );
            })}
