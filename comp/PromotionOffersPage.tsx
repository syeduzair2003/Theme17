...
import { faAngleRight, faBolt, faCheck, faGreaterThan, FontAwesomeIcon } from '@/constants/icons';
...
const PromotionOffersPage = async ({ params }: { params: string }) => {
    ...
    return (
        <>
            <BreadcrumbSection title={promotion?.promotion?.name}
                breadcrumbs={[
                    { label: 'Home', href: '/' },
                    { label: 'Promotions', href: `/${companyData?.promotion_slug}` },
                    { label: promotion?.promotion?.name, href: `/${companyData?.promotion_slug}/${slug}` },
                ]}
            />
                                    ...
                                    <div className="relative bg-white/90 backdrop-blur-xl rounded-[2.5rem] p-8 lg:p-10 shadow-[0_10px_40px_rgba(0,0,0,0.02)] border border-white/50 overflow-hidden">
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            {suggestedPromotions.map((suggestedPromotion, i) => (
                                                <Link
                                                    key={i}
                                                    href={getPromotionHref(suggestedPromotion, companyData?.promotion_slug)}
                                ...
                                {promotion.merchants.slice(0, 12).map((merchant) => (
                                    ...
                                    >
                                        <div className="relative w-full h-16">
                                            <Image
                                                src={getBaseImageUrl(companyData?.domain, merchant.merchant_logo, '')}
